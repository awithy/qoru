...
MESSAGE="${QORU_DEMO_MESSAGE:-qoru-encrypted-multihop-ping}"
...
echo "==> starting E2E egress relay-b on $RELAY_B_ADDR"
go run ./cmd/qoru server -c "$tmpdir/relay-b.yaml" >"$tmpdir/relay-b.log" 2>&1 &
pids+=("$!")
wait_for_log "$tmpdir/relay-b.log" "server listening"

echo "==> starting relay-a on $RELAY_A_ADDR"
go run ./cmd/qoru server -c "$tmpdir/relay-a.yaml" >"$tmpdir/relay-a.log" 2>&1 &
pids+=("$!")
wait_for_log "$tmpdir/relay-a.log" "server listening"
...
echo "==> sending test payload through encrypted qoru multi-hop route"
response="$(send_tcp "$CLIENT_ADDR")"
if [[ "$response" != "$MESSAGE" ]]; then
  echo "expected response $MESSAGE, got $response" >&2
  for f in echo relay-a relay-b client; do echo "--- $f.log ---" >&2; cat "$tmpdir/$f.log" >&2 || true; done
  exit 1
fi

wait_for_log "$tmpdir/client.log" "e2e handshake complete"
wait_for_log "$tmpdir/relay-b.log" "e2e handshake complete"

if grep -q "$MESSAGE" "$tmpdir/relay-a.log" "$tmpdir/relay-b.log" "$tmpdir/client.log" 2>/dev/null; then
  echo "unexpected plaintext demo payload found in qoru logs" >&2
  for f in relay-a relay-b client; do echo "--- $f.log ---" >&2; cat "$tmpdir/$f.log" >&2 || true; done
  exit 1
fi

echo "ok: received echoed payload through encrypted qoru multi-hop route: $response"
