...
MESSAGE="${QORU_DEMO_MESSAGE:-qoru-e2e-auto-direct-ping}"
...
echo "==> starting E2E-capable one-hop server on $SERVER_ADDR"
go run ./cmd/qoru server -c "$tmpdir/server.yaml" >"$tmpdir/server.log" 2>&1 &
pids+=("$!")
wait_for_log "$tmpdir/server.log" "server listening"

echo "==> starting qoru client with e2e:auto on $CLIENT_ADDR"
go run ./cmd/qoru client -c "$tmpdir/client.yaml" >"$tmpdir/client.log" 2>&1 &
pids+=("$!")
wait_for_tcp "$CLIENT_ADDR"

echo "==> sending test payload through direct e2e:auto qoru route"
response="$(send_tcp "$CLIENT_ADDR")"
if [[ "$response" != "$MESSAGE" ]]; then
  echo "expected response $MESSAGE, got $response" >&2
  for f in echo server client; do echo "--- $f.log ---" >&2; cat "$tmpdir/$f.log" >&2 || true; done
  exit 1
fi

if grep -q "e2e handshake complete" "$tmpdir/server.log" "$tmpdir/client.log" 2>/dev/null; then
  echo "unexpected E2E handshake for direct e2e:auto route" >&2
  echo "--- server.log ---" >&2; cat "$tmpdir/server.log" >&2 || true
  echo "--- client.log ---" >&2; cat "$tmpdir/client.log" >&2 || true
  exit 1
fi

echo "ok: e2e:auto skipped E2E frames for direct one-hop route and echoed payload: $response"
