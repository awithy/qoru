...
MESSAGE="${QORU_DEMO_MESSAGE:-qoru-multihop-ping}"
...
echo "==> starting relay-b on $RELAY_B_ADDR"
...
echo "==> sending test payload through qoru multi-hop route"
response="$(send_tcp "$CLIENT_ADDR")"
if [[ "$response" != "$MESSAGE" ]]; then
  echo "expected response $MESSAGE, got $response" >&2
  echo "--- echo.log ---" >&2; cat "$tmpdir/echo.log" >&2 || true
  echo "--- relay-a.log ---" >&2; cat "$tmpdir/relay-a.log" >&2 || true
  echo "--- relay-b.log ---" >&2; cat "$tmpdir/relay-b.log" >&2 || true
  echo "--- client.log ---" >&2; cat "$tmpdir/client.log" >&2 || true
  exit 1
fi

echo "ok: received echoed payload through qoru multi-hop route: $response"
