...
cat > "$OUT_DIR/service-ca.cnf" <<'EOF'
[req]
distinguished_name = req_distinguished_name
x509_extensions = v3_ca
prompt = no
...
openssl genrsa -out "$OUT_DIR/service-ca.key" 4096
openssl req -x509 -new -nodes \
  -key "$OUT_DIR/service-ca.key" \
  -sha256 \
  -days "$DAYS" \
  -out "$OUT_DIR/service-ca.crt" \
  -config "$OUT_DIR/service-ca.cnf"
...
keyUsage = critical,digitalSignature,keyEncipherment
extendedKeyUsage = serverAuth,clientAuth
subjectAltName = URI:spiffe://qoru/service/$service
EOF
  ...
  openssl x509 -req \
    -in "$OUT_DIR/$name.csr" \
    -CA "$OUT_DIR/service-ca.crt" \
    -CAkey "$OUT_DIR/service-ca.key" \
    -CAcreateserial \
    -out "$OUT_DIR/$name.crt" \
    -days "$DAYS" \
    -sha256 \
...
gen_service_cert "relay-b-echo" "echo"
gen_service_cert "relay-c-echo" "echo"
